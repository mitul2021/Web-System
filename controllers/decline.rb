post "/decline" do
    ...
    if ( (@role.eql?("mentee") && (session[:user_id].eql?(@pair.mentee_id))) || ( (@role.eql?("mentor") || @role.eql?("adminmentor") ) && (session[:user_id].eql?(@pair.mentor_id))))
        ...
        if Pair.id_exists?(@pairID)            
            ...
            response.set_cookie("cancel-popup", value: 'true') if session[:user_type].eql?("mentor") || session[:user_type].eql?("adminmentor")
        end
    end
    redirect "/index"
end
